...
public class Vetor1 {
    ...
    private long[] randVet;
    private long[] tempVet;

    public Vetor1(int size) {
        this.randVet = new long[size];
    }

    public long[] randomizeVet() {
    ...
    public void selectionSort() {
        for (int i = 0; i < randVet.length - 1; i++) {
            ...
            long smallerNumber = randVet[index];
    ...
    public void mergeSortIt() {
        long[] from = randVet, to = new long[randVet.length];
    ...
    public void mergeI(long[] from, long[] to, int lo, int mid, int hi) {
    ...
    public void mergeSortRec(int p, int n, long vet[]) {
        if (p < n - 1) {
            int q = (p + n) / 2;
